...
void ring_vm_restorestate(VM* pVM,List* pList,int nPos,int nFlag)
{
   ...
   pVM->nInsideBraceFlag = (char) ring_list_getint(pList,22);
...
void ring_vm_restorestate2(VM* pVM,List* pList,int x)
{
   ...
   pVM->nInsideBraceFlag = (char) ring_list_getint(pList,x + 6);
   ...
   pVM->nPrivateFlag = (char) ring_list_getint(pList,x + 12);

   /* Restore nCallClassInit */
   pVM->nCallClassInit = (char) ring_list_getint(pList,x + 13);
}
...
void ring_vm_restorestack(VM* pVM,List* pList)
{
   ...
   for (x = ring_list_getsize(pList); x >= 1; x--)
   {
      ...
      else if (ring_list_islist(pList,x))
      {
         ...
         RING_VM_STACK_OBJTYPE = (char) ring_list_getint(pList2,2);
      }
   }
